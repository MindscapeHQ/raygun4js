...
g&&(e.disablePulse=!1),e.from="onLoad",m.init(d,e,null)),f){m.attach(),c=a[a.RaygunObject].q;for(var i in c)m.send(c[i].e,{handler:"From Raygun4JS snippet global error handler"})}else if("function"==typeof a.onerror){var j=a.onerror.toString();j.indexOf(l[0])!==-1&&j.indexOf(l[1])!==-1&&(a.onerror=null)}for(var n in k)p(k[n]);k=[],a[a.RaygunObject]=function(){return p(arguments)},a[a.RaygunObject].q=c};"complete"===document.readyState?q():a.addEventListener?a.addEventListener("load",q):a.attachEvent("onload",q)}}(window,window.__instantiatedRaygun);try{delete window.__instantiatedRaygun}catch(e){window.__instantiatedRaygun=void 0}
